...
my $file, $len, $prot, $vars, $cmd, $perc, $apos, $plus, $cdot, $back, 
   $oparen, $cparen, $forward, $lt, $gt, $label, $uri;

while($line = <INP>) {
  chomp($line);
  parse_line($line);
  print FOUT "$uri $cmd $prot $len $vars $perc $apos $plus $cdot $back $oparen $cparen $forward $lt $gt $label\n";
}
...
sub parse_line {
  ...
  # The label is always the last field
  $label = $parts[$cnt - 1];
