...
Webdata::Webdata(char *inf, int f) {
        ...
        {
                /* parsing line by line, as compared with file parsing */
                lines = 1;
...
/* modified to accept strictly integer output of capture/dump algos */
unsigned int Webdata::translateCommand(const char *str) {
        unsigned int ret = 0;

        if((strcmp(str, "GET") == 0) || (strcmp(str, (const char *)1) == 0)) ret += 1;
        else if((strcmp(str, "POST") == 0) || (strcmp(str, (const char *)2) == 0)) ret += 2;
        else if((strcmp(str, "HEAD") == 0) || (strcmp(str, (const char *)4) == 0)) ret += 4;
        else ret += 8;
        return ret;
}
